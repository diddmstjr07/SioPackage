...
class KoreanWordGenerator:
    def __init__(self):
        ...
        self.words = [
            "사과", "바나나", "딸기", "수박", "복숭아", "참외",
            "감자", "고구마", "당근", "오이", "토마토", "파프리카",
            "배추", "양배추", "상추", "무", "미나리", "양파",
            "감", "귤", "오렌지", "레몬", "라임", "자두",
            "수박", "딸기", "블루베리", "레드포도", "키위", "망고",
            "체리", "파인애플", "포도", "자몽", "파파야", "배",
            "바베큐", "삼계탕", "김밥", "비빔밥", "초밥", "된장찌개",
            "부대찌개", "김치찌개", "갈비찜", "제육볶음", "불고기", "떡볶이",
            "김치", "멸치볶음", "어묵", "라면", "만두", "닭강정",
            "치킨", "피자", "햄버거", "샌드위치", "핫도그", "타코",
            "브리또", "푸드트럭", "아이스크림", "케이크", "파이", "마카롱",
            "도넛", "쿠키", "브라우니", "초콜릿", "치즈케이크", "프로즌요거트",
            "요거트", "베이컨", "계란", "토스트", "핫케이크", "와플",
            "토스트", "핫케이크", "와플", "프렌치토스트", "베이글", "크루아상",
            "크로와상", "머핀", "통밀빵", "식빵", "바게트", "파운드케이크"
        ]
        self.random = random

    def get_random_word(self):
        return self.random.choice(self.words)
...
class RunManage: # 선언
    ...
    def run(self, data):
        compare = CompareSimilaity() # 호출 # 메뉴가 뭐가 있을까요?, 아메리카노 한 잔과 블루베리 치즈케이크 먹고 싶어요, 포장할게요
        word = KoreanWordGenerator()
        compare.delay_blocker() # 함수 호출
        # input("Enter to start") # Enter Key 입력 기다리기
        count = 0
        timer = time.time()
        while True:
            question = word.get_random_word()
            print("--------------------------------------------------------------")
            start_time = time.time() # 타이머 셋
            compare.getdata(self.thread) # 지정한 thread 개수 매개변수로 getdata 함수 호출
            compare.thread(question) # 위에서 인스턴스로써 저장해준 변수를 활용하여 thread 함수 호출
            end_time = time.time() # 타이머 종료
            print(f"최종 소요 시간: {end_time - start_time}") # 위의 작업 전체 소요 시간 출력
            if end_time - start_time > 0.5:
                count += 1
                end_timer = time.time()
                print("지연 횟수: " + str(count))
                print("지연 간격: " + str(end_timer - timer))
                time.sleep(4)
            timer = time.time()
            # input("Enter to restart")
            predict_key, predict_value, ques = compare.__display__(question) # __display__ 호출
            print(f"\n고유 질문: {ques}") # 출력
            print(f"예측 분석 질문: {predict_key}") # 출력
            print(f"예측 분석 답변: {predict_value}") # 출력
